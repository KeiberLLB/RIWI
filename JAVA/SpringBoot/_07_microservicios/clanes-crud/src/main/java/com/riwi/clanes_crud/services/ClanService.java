...
public class ClanService implements IClanService {
  ...
  public Clan update(Long id, ClanUpdateReq req) {
    Clan clan = this.clanRepository.findById(id).orElseThrow(() -> new RuntimeException("Cohort not found"));
    if (req.getCohortId() != clan.getCohort().getId()) {
      Cohort cohort = this.cohortRepository.findById(req.getCohortId()).orElseThrow(
          () -> new RuntimeException("Cohort not found"));
      clan.setCohort(cohort);
    }

    clan.setName(req.getName());
    clan.setDescription(req.getDescription());
    clan.setUpdateAt(LocalDateTime.now());
